"""Tests for the heuristic model router."""
...
class TestHeuristicRouter:
    ...
    def test_code_prefers_coder(self) -> None:
        _register_models()
        router = HeuristicRouter(available_models=["small", "large", "coder"])
        ctx = RoutingContext(query="def foo():", query_length=10, has_code=True)
    ...
    def test_high_urgency_overrides_to_small(self) -> None:
        _register_models()
        router = HeuristicRouter(available_models=["small", "large", "coder"])
        ctx = RoutingContext(query="x" * 501, query_length=501, urgency=0.9)
        assert router.select_model(ctx) == "small"
    ...
    def test_fallback_chain(self) -> None:
        ...
        ctx = RoutingContext(query="Tell me about cats", query_length=60)
        assert router.select_model(ctx) == "large"
